...
export default function CodeReviewPage() {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-slate-950 flex items-center justify-center z-50">
        <div className="text-xs font-mono text-slate-700 tracking-widest animate-pulse">Loading code…</div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-[#0d0d14] z-50">

      <div className="flex items-center gap-3 px-4 h-11 border-b border-slate-800 bg-[#0d0d14] shrink-0">
        <button
          onClick={() => router.back()}
      ...
      <div className="flex flex-1 overflow-hidden">
        <div
          className="relative flex flex-col border-r border-slate-800 bg-[#0a0a12] shrink-0 transition-[width] duration-200"
        ...
        >
          ...
          {aiPanelCollapsed && (
            <div className="flex-1 flex items-center justify-center overflow-hidden">
              <span
                className="text-xs font-mono text-slate-700 uppercase tracking-widest select-none"
          ...
          {!aiPanelCollapsed && (
                      ...
                      {[
                        ...
                        <div key={key}>
                          ...
                          <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${color} transition-all duration-500`} style={{ width: `${(submission.aiBreakdown as any)[key]}%` }} />
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {submission.featuresCompleted.map((f, i) => (
                        <span key={i} className="text-xs font-mono px-1.5 py-0.5 bg-emerald-950/50 border border-emerald-900/50 text-emerald-400 rounded">
                          ✓ {f}
                        </span>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {submission.featuresMissing.map((f, i) => (
                        <span key={i} className="text-xs font-mono px-1.5 py-0.5 bg-red-950/50 border border-red-900/50 text-red-400 rounded">
                          ✗ {f}
                        </span>
                      ))}
                    </div>
            ...
            >
                ...
                style={{
                    ...
                    display: "flex",
                    flexDirection: "row",
                    // ← remove overflow restrictions from layout
                }}
                >
                <SandpackFileExplorer
                    style={{
                ...
                {/* Editor — key: set height explicitly via style */}
                ...
                {/* Preview — fix with iframe height */}
                <div style={{ flex: 1, height: "100%", minWidth: 0, overflow: "hidden" }}>
                    <SandpackPreview
                    showRefreshButton
